...
# connect to db, return pconn_rsql client obj
drv = RPostgreSQL::PostgreSQL()
# drv <- DBI::dbDriver("PostgreSQL");d
db_conn <- DBI::dbConnect(drv,
                          host = host,
                          port = port,
                          user = user,
                          password = password,
...
# build query, with limit if given
sql_query = "
...
# execute query
result_df = DBI::dbGetQuery(db_conn, sql_query)
print(head(result_df, 10))

# save out
outpath = "/Users/bradgowland/dev/medqip.connect/scratch/simpl2_evals.csv"
write.csv(result_df, outpath, row.names=FALSE)
